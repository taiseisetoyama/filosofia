...
try:
    ...
    st.error("Secrets（API_KEY, SHEET_URL, GAS_URL）を設定してください。")
    st.stop()

genai.configure(api_key=API_KEY)

# 【重要】2.5-flash（20回制限）を避け、1.5-flash（1500回枠）を直接指定
...
with col_mem:
    ...
    st.write(f"使用中: {MODEL_NAME} (1500回/日枠)")
    st.write("現在の記憶量:", len(st.session_state.full_memory))
